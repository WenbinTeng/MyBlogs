...
Vivado 提供的 Dynamic Function eXchange（DFX）功能是面向异构计算平台中模块级资源动态调度的重要技术手段。本实验将介绍 Vivado 中基于脚本的 DFX 流程：首先，我们将用脚本单独综合所有的静态模块（Static Modules）和每个可重构模块的变体（Variants for each Reconfigurable Partitions）；然后，在脚本中编写可重构区域的约束（Pblocks），并实现该设计的初始配置；接着，我们将锁定设计的静态部分、使用变体更新可重构模块、重新运行实现可选配置；最后，我们将验证每个实现的可重构模块是否与设计的静态部分兼容，如果兼容，则生成比特流。

可以看出，DFX 的流程依然是基于 Vivado 的基本编译流程，即“综合-实现（布局布线）-生成比特流”。在此基础上，DFX 需要将设计划分为静态设计和动态重构设计，每一个静态设计与动态重构设计都需要经过前述的基本编译流程，以生成最终运行的比特流。此外，还需要在编译流程中加入一些检查过程，保证静态设计与动态重构设计之间的兼容性。

目前在 Vivado 中主要基于脚本进行 DFX 设计，本篇博客将介绍其基本设计流程。
